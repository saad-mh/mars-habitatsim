...
if ! conda env list | grep -q "^qwen_vlm "; then
    ...
    echo "First-frame goal selection still uses one VLM call (qwen_server subprocess, transformers + Qwen2.5-VL) even though NavDP drives every other frame."
    exit 1
fi
...
echo "[inf] Starting run_navdp_rollout"
echo ""
