...
class NavDPS2DiffClient:
    ...
    def plan(
        ...
        *,
        ...
        selected_energy_valid = bool(
            diagnostics.get(
                "selected_energy_valid",
                [
                    not (
                        bool(diagnostics["fallback_stop"][0])
                        or bool(diagnostics["escape_turn"][0])
                    )
                ],
            )[0]
        )
        invalid_energy = float("nan")
        ...
        return NavDPS2DiffOutput(
            ...
            selected_energy_valid=selected_energy_valid,
            selected_barrier_energy=(
                float(diagnostics["selected_barrier_energy"][0])
                if selected_energy_valid
                else invalid_energy
            ),
            selected_circulation_energy=(
                float(diagnostics["selected_circulation_energy"][0])
                if selected_energy_valid
                else invalid_energy
            ),
            ...
            selected_lyapunov_energy=(
                float(diagnostics.get("selected_lyapunov_energy", [0.0])[0])
                if selected_energy_valid
                else invalid_energy
            ),
        )
...
def start_server(args: argparse.Namespace) -> Optional[subprocess.Popen[Any]]:
    ...
    command = [
        ...
        "--terminal-goal-weight",
        str(args.terminal_goal_weight),
        "--lyapunov-weight",
        str(args.lyapunov_weight),
        "--nominal-weight",
        str(args.nominal_weight),
        "--smoothness-weight",
        str(args.smoothness_weight),
        "--step-weight",
        str(args.step_weight),
        "--hard-collision-penalty",
        str(args.hard_collision_penalty),
        "--barrier-weight",
    ...
    particle_flags = {
        ...
        "hard-collision-rejection": args.hard_collision_rejection,
        "deterministic-escape": args.deterministic_escape,
...
def parser() -> argparse.ArgumentParser:
    ...
    argument_parser.add_argument("--terminal-goal-weight", type=float, default=1.5)
    argument_parser.add_argument("--lyapunov-weight", type=float, default=4.0)
    argument_parser.add_argument("--nominal-weight", type=float, default=0.35)
    argument_parser.add_argument("--smoothness-weight", type=float, default=0.15)
    argument_parser.add_argument("--step-weight", type=float, default=20.0)
    argument_parser.add_argument("--hard-collision-penalty", type=float, default=1.0e4)
    argument_parser.add_argument(
...
def main() -> None:
    ...
    try:
        ...
        if args.obstacle_mode == "mesh" and args.obstacle_world_xz:
            ...
            mesh_placed = True
            requested_mesh_count = len(args.obstacle_world_xz)
            if len(mesh_objects) != requested_mesh_count:
                raise RuntimeError(
                    "world obstacle registration mismatch: "
                    f"requested={requested_mesh_count}, registered={len(mesh_objects)}"
                )
            print(
                f"[world-mesh] registered_obstacles={len(mesh_objects)} "
                f"requested_obstacles={requested_mesh_count}",
                flush=True,
            )
        ...
        for step in range(int(args.max_steps)):
            ...
            if args.obstacle_mode == "depth":
                ...
                visible_obstacle_mesh_count = sum(
                    int(np.any(semantic == semantic_id)) for semantic_id in semantic_ids
                )
            ...
            rows["selected_energy_valid"].append(result.selected_energy_valid)
            ...
            rows["visible_obstacle_mesh_count"].append(visible_obstacle_mesh_count)
            ...
            if args.save_frames and step % max(int(args.save_every), 1) == 0:
                ...
                label = (
                    ...
                    f"meshes={visible_obstacle_mesh_count}/{len(mesh_objects)} "
            ...
            print(
                ...
                f"meshes_visible={visible_obstacle_mesh_count}/{len(mesh_objects)} "
        ...
        np.savez_compressed(
            ...
            **{
            ...
            hard_collision_penalty=np.asarray(
                args.hard_collision_penalty, dtype=np.float64
            ),
            hard_collision_rejection=np.asarray(args.hard_collision_rejection),
            deterministic_escape=np.asarray(args.deterministic_escape),
            safety_weight=np.asarray(args.safety_weight, dtype=np.float64),
            terminal_goal_weight=np.asarray(
                args.terminal_goal_weight, dtype=np.float64
            ),
            nominal_weight=np.asarray(args.nominal_weight, dtype=np.float64),
            smoothness_weight=np.asarray(args.smoothness_weight, dtype=np.float64),
            step_weight=np.asarray(args.step_weight, dtype=np.float64),
            circulation_weight=np.asarray(args.circulation_weight, dtype=np.float64),
            circulation_switch_weight=np.asarray(
                args.circulation_switch_weight, dtype=np.float64
            ),
