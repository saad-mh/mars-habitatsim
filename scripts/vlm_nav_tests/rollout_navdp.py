        ...
        for step in range(int(args.max_steps)):
            ...
            if mesh_tracking_mode:
                ...
                if int(_gm.sum()) >= int(args.lost_goal_min_px):
                    if (
                        belief_g is None
                    ):  # seed ONCE from the mask, then dead-reckon (stable distance)
                ...
                if grounder is not None and (
                    belief_g is None or step % max(1, int(args.grounder_every)) == 0
                ):
            ...
            if args.qwen_steer:
                ...
                near_goal_now = goal_dist_now <= float(args.stop_dist) + float(
                    args.cbf_deadzone
                )
                if near_goal_now:
                    ...
                    force_side = (
                        1.0
                        if intent == "left"
                        else (-1.0 if intent == "right" else None)
                    )
